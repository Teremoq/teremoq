...
mkdir -p -- "$(dirname -- "${output}")"
{
    ...
    for key in clock_error_ms ingest_to_publish_p95_ms ingest_to_publish_source network_subscriber_p95_ms network_subscriber_source frames_presented media_objects_received media_bytes_received media_session_source rx_to_canvas_p95_ms rx_to_canvas_source g2g_measurement_status glass_to_glass_p95_ms glass_to_glass_source icmp_echo_loss_percent_approximation icmp_echo_jitter_ms_approximation network_probe_source server_cpu_peak_percent server_memory_peak_mib client_cpu_peak_percent client_memory_peak_mib server_bandwidth_peak_mbps client_bandwidth_peak_mbps host_resource_source wifi_recovery_status wifi_recovery_armed wifi_loss_observed wifi_recovery_observed wifi_recovery_ms wifi_recovery_provenance wifi_recovery_source session_loss_count session_recovery_count defender_firewall_rule_count_during_run hyperv_firewall_rule_count_during_run cleanup_processes cleanup_defender_firewall_rules cleanup_hyperv_firewall_rules network_probe_kind server_collector_sha256 client_collector_sha256 browser_observation_sha256 player_collector_sha256; do
        printf '| `%s` | `%s` |\n' "${key}" "${LAN_RESULTS[${key}]}"
    done
    ...
    if [[ "${LAN_RESULTS[g2g_measurement_status]}" != measured || "${LAN_RESULTS[clocks_calibrated]}" != true ]]; then
        printf 'Glass-to-glass is not claimable unless Web measured it from valid source timecode and clocks are calibrated.\n'
    else
