...
foreach ($executable in @($gitPath, $nodePath)) {
    if (-not (Test-Path -LiteralPath $executable -PathType Leaf) -or ((Get-Item -LiteralPath $executable -Force).Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
        throw "Required reviewed executable is unavailable: $executable"
    }
}
...
try {
    $head = (@(& $gitPath @gitPrefix rev-parse HEAD 2>$null) -join '').Trim()
    $branch = (@(& $gitPath @gitPrefix symbolic-ref --short HEAD 2>$null) -join '').Trim()
    $remote = (@(& $gitPath @gitPrefix remote get-url origin 2>$null) -join '').Trim().TrimEnd('/')
    $dirty = (@(& $gitPath @gitPrefix status --porcelain=v1 --untracked-files=normal 2>$null) -join '').Trim()
} finally {
    $env:GIT_CONFIG_NOSYSTEM = $oldNoSystem
    $env:GIT_CONFIG_GLOBAL = $oldGlobal
}
if ($LASTEXITCODE -ne 0 -or $head -cne $ExpectedCommit -or $branch -cne 'codex/lan-e2e-integration' -or
