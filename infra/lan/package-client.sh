...
repo='' commit='' config='' player_dir='' certificate='' fingerprint='' output_dir='' git_url='' git_ref='' git_subdirectory=''
while (( $# > 0 )); do
    case "$1" in
        --repo|--commit|--config|--player-dir|--certificate|--fingerprint|--output-dir|--git-url|--git-ref|--git-subdirectory)
            ...
            case "$1" in
                ...
                --git-url) git_url="$2" ;; --git-ref) git_ref="$2" ;; --git-subdirectory) git_subdirectory="$2" ;;
            esac
            shift 2 ;;
...
printf 'schema_version\t1\nrepository_url\t%s\nrepository_ref\t%s\nrepository_subdirectory\t%s\nallowed_client_commit\t%s\nsource_commit\t%s\npackage_version\t%s\nclient_protocol_version\tteremoq-lan-git-v1\nplayer_manifest_sha256\t%s\nlauncher_contract_sha256\t%s\nlan_config_sha256\t%s\n' \
    "${git_url}" "${git_ref}" "${git_subdirectory}" "${commit}" "${commit}" "${package_version}" \
