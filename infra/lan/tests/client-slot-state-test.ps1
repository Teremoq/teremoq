...
function New-FixtureSlot {
    param([string]$Commit, [string]$IdentityHex, [string]$Tree, [string]$Lock)
    ...
    $record = New-TeremoqLanSlotRecord -UpdaterCommit $Commit -PlayerIdentity "sha256:${IdentityHex}" `
...
try {
    $first = New-FixtureSlot -Commit ('1' * 40) -IdentityHex ('2' * 64) -Tree ('3' * 40) -Lock ('4' * 64)
    ...
    $second = New-FixtureSlot -Commit ('5' * 40) -IdentityHex ('6' * 64) -Tree ('7' * 40) -Lock ('8' * 64)
    ...
    if (Test-Path -LiteralPath (Join-Path $root $first.player_relative_path)) { throw 'obsolete player was not cleaned' }
    ...
    $third = New-FixtureSlot -Commit ('e' * 40) -IdentityHex ('6' * 64) -Tree ('7' * 40) -Lock ('8' * 64)
    ...
    if (@(Get-ChildItem -LiteralPath (Join-Path $root 'players') -Directory).Count -ne 1) {
    ...
    $tampered = New-FixtureSlot -Commit ('9' * 40) -IdentityHex ('a' * 64) -Tree ('b' * 40) -Lock ('c' * 64)
    ...
    Write-Output 'client slot state tests passed: reuse, update, atomic activation, rollback, cleanup, config preservation, locking, interruption recovery, tamper rejection'
} finally {
