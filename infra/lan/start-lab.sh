...
while (( $# > 0 )); do
    case "$1" in
        --config|--commands|--authorization|--server-preflight|--client-preflight|--firewall-attestation|--certificate|--key|--fingerprint|--identity-profile|--proxy-attestation|--artifact-root|--state-dir)
            (( $# >= 2 )) || lan_die "$1 requires a value"; options["${1#--}"]="$2"; shift 2 ;;
        *) lan_die "unknown lab start argument: $1" ;;
    esac
done
for key in config commands authorization server-preflight client-preflight firewall-attestation certificate key fingerprint identity-profile proxy-attestation artifact-root state-dir; do
...
for path in commands authorization server-preflight client-preflight firewall-attestation certificate key fingerprint identity-profile proxy-attestation artifact-root state-dir; do
