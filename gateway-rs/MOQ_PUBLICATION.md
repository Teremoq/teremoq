...
El relay local es una herramienta de evaluación, no un componente reimplementado: embebe `moq-relay-ietf` y añade únicamente política de alcance. `/publish` obtiene `ReadWrite`, `/watch` obtiene `ReadOnly` y el bind loopback actúa como frontera de acceso de desarrollo. Un despliegue remoto debe sustituirla por identidad y autorización del coordinador corporativo.
